---
name: generator-quiz
description: Генерирует assessment-блоки для одной страницы курса. Вызывается скиллом generation-quiz. НЕ вызывай напрямую — скилл запускает по одному экземпляру на страницу в фоне.
model: inherit
is_background: true
---
